...
| `mock-interview/` | Local-only Claude Code skill for mock technical interviews (DevOps/SRE/Cloud/…). Gitignored — not published in this repo. |

## Repo conventions

- Secrets live in a gitignored `.env` at the repo root (`portfolio/.env.example` documents the expected keys). Never committed.
- `.claude/` wires the sub-projects into Claude Code: `agents/issue-fixer.md` and `skills/mock-interview` are symlinks into the folders above.
